...
<form class="form-inline" action="/admin/bri" method="GET">
  <div class="form-group">
    ...
    <a class="btn bg-success" href="{{ url('admin/bri') }}"><li class="fa fa-spinner fa-spin"></li></a>
  </div>
</form>
<br>
            ...
            <table class="table table-bordered" id="table-1">
              <thead>
                <tr>
                  ...
                  <th class="bg-info" style="color:black">Kode Teller</th>
              ...
              <tbody>
                @foreach($bri as $data)
                    <tr>
                        ...
                        <td >
                            <button type="button" class="btn btn-success" data-toggle="modal" data-target="#edit-bri" data-id="{{ $data->id }}" data-no_urut="{{ $data->no_urut }}" data-tanggal_1="{{ $data->tanggal_1 }}" data-tanggal_2="{{ $data->tanggal_2 }}" data-remark="{{ $data->remark }}" data-kode_teller="{{ $data->kode_teller }}" data-debit="{{ $data->debit }}" data-kredit="{{ $data->kredit }}" data-saldo="{{ $data->saldo }}" data-kode_rekening_id="{{ $data->kode_rekening->nama }}"><i class="fa fa-pen"></i></button>
                            <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#delete-bri" data-id="{{ $data->id }}" data-no_urut="{{ $data->no_urut }}" data-tanggal_1="{{ $data->tanggal_1 }}" data-tanggal_2="{{ $data->tanggal_2 }}" data-remark="{{ $data->remark }}" data-kode_teller="{{ $data->kode_teller }}" data-debit="{{ $data->debit }}" data-kredit="{{ $data->kredit }}" data-saldo="{{ $data->saldo }}" data-kode_rekening_id="{{ $data->kode_rekening->nama }}"><i class="fa fa-trash"></i></button>
                        </td>
                    </tr>
                @endforeach
              </tbody>
