...
@section('desc') Kumpulan data rekap BSM @endsection
@section('header-2') BSM @endsection


@section('content')

<form class="form-inline" action="/admin/bsm-rekap-tahun" method="GET">
  <div class="form-group">
    ...
    <a style="color:black" href="/admin/bsm-rekap-tahun" class="btn bg-warning" role="button" aria-pressed="true"><li class="fas fa-arrow-left"></li></li></a>
  </div>
</form>
<br>
            ...
            <tbody>
              @foreach($kode_transaksi_id as $data)
                  <tr>
                      ...
                      <?php 
                      $hitung = DB::table('kode_transaksis')->selectRaw('count(bsm_data.kode_transaksi_id) AS jml_count')
                      ->join('bsm_data','bsm_data.kode_transaksi_id','=','kode_transaksis.id')
                      ->where('kode_transaksi_id', '=', $data->id)
                      ->get();

                      $debit = DB::table('kode_transaksis')->selectRaw('sum(bsm_data.debit) AS jml_debit')
                      ->join('bsm_data','bsm_data.kode_transaksi_id','=','kode_transaksis.id')
                      ->where('kode_transaksi_id', '=', $data->id)
                      ->get();

                      $kredit = DB::table('kode_transaksis')->selectRaw('sum(bsm_data.kredit) AS jml_kredit')
                      ->join('bsm_data','bsm_data.kode_transaksi_id','=','kode_transaksis.id')
                      ->where('kode_transaksi_id', '=', $data->id)
                      ->get();
                      ?>
