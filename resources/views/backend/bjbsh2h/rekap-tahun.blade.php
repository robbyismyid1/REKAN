...
@section('desc') Kumpulan data rekap BJBS H2H @endsection
@section('header-2') BJBS H2H @endsection


@section('content')

<form class="form-inline" action="/admin/bjbs-h2h-rekap-tahun" method="GET">
  <div class="form-group">
    ...
    <a style="color:black" href="/admin/bjbs-h2h-rekap-tahun" class="btn bg-warning" role="button" aria-pressed="true"><li class="fas fa-arrow-left"></li></li></a>
  </div>
</form>
<br>
            ...
            <tbody>
              @foreach($kode_transaksi_id as $data)
                  <tr>
                      ...
                      <?php 
                      $hitung = DB::table('kode_transaksis')->selectRaw('count(bjbsh2h_data.kode_transaksi_id) AS jml_count')
                      ->join('bjbsh2h_data','bjbsh2h_data.kode_transaksi_id','=','kode_transaksis.id')
                      ->where('kode_transaksi_id', '=', $data->id)
                      ->get();

                      $debit = DB::table('kode_transaksis')->selectRaw('sum(bjbsh2h_data.debit) AS jml_debit')
                      ->join('bjbsh2h_data','bjbsh2h_data.kode_transaksi_id','=','kode_transaksis.id')
                      ->where('kode_transaksi_id', '=', $data->id)
                      ->get();

                      $kredit = DB::table('kode_transaksis')->selectRaw('sum(bjbsh2h_data.kredit) AS jml_kredit')
                      ->join('bjbsh2h_data','bjbsh2h_data.kode_transaksi_id','=','kode_transaksis.id')
                      ->where('kode_transaksi_id', '=', $data->id)
                      ->get();
                      ?>
