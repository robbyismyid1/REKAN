            ...
            <tbody>
              @foreach($kode_transaksi_id as $data)
                  <tr>
                      ...
                      <?php 
                      $hitung = DB::table('kode_transaksis')->selectRaw('count(bris_data.kode_transaksi_id) AS jml_count')
                      ->join('bris_data','bris_data.kode_transaksi_id','=','kode_transaksis.id')
                      ->where('kode_transaksi_id', '=', $data->id)
                      ->get();

                      $debit = DB::table('kode_transaksis')->selectRaw('sum(bris_data.debit) AS jml_debit')
                      ->join('bris_data','bris_data.kode_transaksi_id','=','kode_transaksis.id')
                      ->where('kode_transaksi_id', '=', $data->id)
                      ->get();

                      $kredit = DB::table('kode_transaksis')->selectRaw('sum(bris_data.kredit) AS jml_kredit')
                      ->join('bris_data','bris_data.kode_transaksi_id','=','kode_transaksis.id')
                      ->where('kode_transaksi_id', '=', $data->id)
                      ->get();
                      ?>
